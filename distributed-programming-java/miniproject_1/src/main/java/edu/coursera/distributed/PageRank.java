    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks = sites.join(ranks)
                .flatMapToPair(kv -> {
                    ...
                    Iterator<Integer> edgeIterator = edges.edgeIterator();

                    while (edgeIterator.hasNext()) {
                        final int target = edgeIterator.next();
                        contribs.add(new Tuple2<>(target, currentRank / (double) edges.getNEdges()));
                    }

                    return contribs;
                });
        return newRanks.reduceByKey((Double r1, Double r2) -> r1 + r2).mapValues(y -> 0.15 + 0.85 * y);
    }
}
